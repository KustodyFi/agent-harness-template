...
for file in AGENTS.md agent.project.md README.md .cowork/STATUS.md .agent.default/shared/log.md; do
  if [ -f "$file" ]; then
    ...
    "${SED_INPLACE[@]}" "s/{{PROJECT_NAME_LOWER}}/$PROJECT_NAME_LOWER/g" "$file"
    "${SED_INPLACE[@]}" "s/{{PROJECT_DESCRIPTION}}/$PROJECT_DESCRIPTION/g" "$file"
    "${SED_INPLACE[@]}" "s/{{TECH_STACK}}/$TECH_STACK/g" "$file"
    "${SED_INPLACE[@]}" "s/{{CURRENT_PHASE}}/$CURRENT_PHASE/g" "$file"
